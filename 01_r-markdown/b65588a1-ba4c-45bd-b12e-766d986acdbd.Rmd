---
title: "Open Government Data, opendata.swiss"
date: "2024-01-29"
output: html_document
---

## Dataset: Respiratorische Viren

Daten zur Verbreitung und zeitlichen Entwicklung von respiratorischen Virusinfektionen und Erkrankungsfällen in der Schweiz und im Fürstentum Liechtenstein. Sie finden Daten zu ambulanten Konsultationen aus dem Sentinella-Meldesystem und zur Viruslast im Abwasser aus dem Abwassermonitoring.

[Direct link by **opendata.swiss** for dataset](https://opendata.swiss/de/dataset/respiratorische-viren)<br>
[Direct link by **Abteilung Übertragbare Krankheiten** for dataset](idd.bag.admin.ch)

Auto generated R starter code for data set respviruses@abteilung-uebertragbare-krankheiten.

## Metadata

- **Publisher** `Bundesamt für Gesundheit, Abteilung Übertragbare Krankheiten`
- **Organization.display_name.de** `Abteilung Übertragbare Krankheiten`
- **Organization.url** `https://www.bag.admin.ch/`
- **Maintainer** `Bundesamt für Gesundheit, Abteilung Übertragbare Krankheiten`
- **Maintainer_email** `idd@bag.admin.ch`
- **Keywords.de** `['rhinovirus', 'bag', 'rsv', 'coronavirus', 'influenza', 'sars-cov-2', 'adenovirus', 'covid-19', 'respiratorische-viren']`
- **Issued** `2023-11-08T00:00:00`
- **Metadata_created** `2023-11-08T07:46:08.583838`
- **Metadata_modified** `2024-01-25T13:33:13.988052`


# Load packages

```{r}

library(tidyverse)
library(skimr)

```

# Load data

-   The dataset has 2 distribution(s) in CSV format.
-   All available CSV distributions are listed below and can be read into a tibble.

```{r}

# Distribution 0
# Package_id               : b65588a1-ba4c-45bd-b12e-766d986acdbd
# Description              : Die hier veröffentlichten Daten basieren auf Informationen aus dem Sentinella-⁠Meldesystem. Im Rahmen dieses Meldesystems übermitteln ambulant tätige Ärztinnen und Ärzte Informationen zu Erstkonsultationen aufgrund akuter respiratorischer Infektion (ARI) oder grippeähnlicher Erkrankung (ILI) in Arztpraxen und bei Hausbesuchen.
# Issued                   : 
# Modified                 : 
# Rights                   : NonCommercialAllowed-CommercialAllowed-ReferenceRequired

df <- read_delim('https://idd.bag.admin.ch/api/v1/export/current/unwrap-csv/RESPVIRUSES_sentinella.zip')

# Distribution 1
# Package_id               : b65588a1-ba4c-45bd-b12e-766d986acdbd
# Description              : Die hier veröffentlichten Daten basieren auf Informationen aus dem nationalen Abwassermonitoring der Schweiz und Fürstentum Liechtenstein. Dieses Monitoring erfasst seit Januar 2022 die Viruslast von SARS-CoV-2 und seit Juli 2023 die Viruslast von Influenza und RSV in Abwasserproben der teilnehmenden Abwasserreinigungsanlagen (ARA).
# Issued                   : 
# Modified                 : 
# Rights                   : NonCommercialAllowed-CommercialAllowed-ReferenceRequired

df <- read_delim('https://idd.bag.admin.ch/api/v1/export/current/unwrap-csv/RESPVIRUSES_wastewater.zip')



```

# Analyze data

```{r}

glimpse(df)
str(df)
skim(df)

head(df)
tail(df)

```

# Continue your code here...

```{r}

```

------------------------------------------------------------------------

# Contact

Bundesamt für Gesundheit, Abteilung Übertragbare Krankheiten | idd@bag.admin.ch
